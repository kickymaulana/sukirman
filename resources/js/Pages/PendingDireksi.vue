...
const applyFilters = () => {
    router.get(baseUrl + '/pending-direksi/' + (props.direksi?.id || ''), {
...
<template>
    <Head :title="'Pending Direksi: ' + (direksi?.name || '')" />
    <div class="layout">
        <var-app-bar :title="'⏳ Pending Direksi' + (direksi?.name ? ': ' + direksi.name : '')" title-position="center">
                ...
                <table class="tbl">
                    ...
                    <tbody>
                        ...
                        <tr v-if="!requests.data.length">
                            <td colspan="7" class="empty">Tidak ada MR Pending Direksi</td>
                        </tr>
                    </tbody>
                </table>
            </div>
