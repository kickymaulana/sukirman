...
interface MR { id: number; mr_number: string; factory: string; user: { name: string }; items: any[]; created_at: string }
const props = defineProps<{ requests: { data: MR[] }; direksiUsers: any[] }>()
...
<template>
    <Head title="Manager - SUKIRMAN" />
    <div class="layout">
        <var-app-bar title="Approval Manager" title-position="center">
        ...
        <main class="content">
            ...
            <div v-for="mr in requests.data" :key="mr.id" class="card" @click="openDetail(mr.id)">
                <div class="card-h"><span class="mr-num">{{ mr.mr_number }}</span><var-chip type="warning" size="mini">Pending</var-chip></div>
