...
interface MR { id: number; mr_number: string; factory: string; user: { name: string }; manager: { name: string }; created_at: string }
...
<template>
    <Head title="FM/GM - SUKIRMAN" />
    <div class="layout">
        <var-app-bar title="Approval FM/GM" title-position="center">
        ...
        <main class="content">
            ...
            <div v-for="mr in requests.data" :key="mr.id" class="card" @click="openDetail(mr.id)">
                <div class="card-h"><span class="mr-num">{{ mr.mr_number }}</span><var-chip type="info" size="mini">Pending</var-chip></div>
                <p><strong>{{ mr.user?.name }}</strong> • {{ mr.factory }} • Forward: {{ mr.manager?.name }}</p>
                <p class="date">{{ mr.created_at }}</p>
            </div>
        </main>
    </div>
</template>
